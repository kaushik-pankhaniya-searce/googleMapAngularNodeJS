...
Handsontable.TableView = function (instance) {
  ...
  var walkontableConfig = {
    ...
    onCellMouseDown: function (event, coords, TD) {
      ...
      /**
       * Added this custom header click event to sort data array bind to handsontable
       * as Handson column sorting only sorts html data visible not model variable
       * @param event
       * @param coords
       * @param TD
       */
    onHeaderMouseDown: function (event, coords, TD) {
          ...
          var mydata = instance.getData();
          var instanceSettings = instance.getSettings();
          var columnName = "";
          var colMeta, colType, rowHeaderCount;
          rowHeaderCount = instanceSettings['rowHeaders'] == undefined ? 0 : 1


          /** Column Number shifts by 1 if row headers are present **/
          if (TD.cellIndex <= instanceSettings['fixedColumnsLeft']) {
              colMeta = instance.getCellMeta(0, TD.cellIndex - rowHeaderCount);
          }
          else {
              colMeta = instance.getCellMeta(0, TD.cellIndex + instance.colOffset() - rowHeaderCount);
          }

          /** Store the sort Order of instance **/
          if (instance.sortOrder == undefined) {
              instance.sortOrder = true;
          }
          var sortedData = {};
          columnName = colMeta['prop'];
          colType = colMeta['type'];
          if( ! (colType == 'checkbox' && colMeta.col == 0 ) ) {
              /** pop only spare rows**/
              for (var cntSpareRows = 0; cntSpareRows < instanceSettings['minSpareRows']; cntSpareRows++) {
                  mydata.pop();
              }
              sortedData = mydata.sort(function (a, b) {
                  ...
                  if (a[columnName] === null || a[columnName] === undefined || a[columnName] === "") {
                      return 1;
                  }
                  if (b[columnName] === null || b[columnName] === undefined || b[columnName] === "") {
                      return -1;
                  }

                  if (colType == 'date') {
                      var aDate = new Date(a[columnName]);
                      var bDate = new Date(b[columnName]);

                      if (aDate < bDate) return instance.sortOrder ? -1 : 1;
                      if (aDate > bDate) return instance.sortOrder ? 1 : -1;
...
//                      console.log(a[columnName] + ',' + b[columnName]);
                      if (a[columnName] < b[columnName]) return instance.sortOrder ? -1 : 1;
                      if (a[columnName] > b[columnName]) return instance.sortOrder ? 1 : -1;
                      return 0;
                  }
              ...
              /** Finally load sorted data in handsontable **/
              instance.loadData(sortedData);
          }
      },
...
HandsontableDateEditorClass.prototype.createElements = function () {
  ...
  this.datePickerStyle.zIndex = 1100;
...
//    for (var i = 0, ilen = this.countRows(); i < ilen; i++) {
      ...
      if (this.sortColumn < instance.getSettings('fixedColumnsLeft').fixedColumnsLeft) {
...
function HandsontableManualColumnResize() {
  ...
  function getColumnWidth(TH) {
    ...
    console.log('Max width  - ' + maxWidth + ' th width  - ' + thWidth)
  ...
  this._bindEvent(listenerElement, 'keydown', function (event) {
    ...
    if (isCtrlDown) {
      if (document.activeElement !== that.elTextarea && (that.getSelectionText() != '' || ['INPUT', 'SELECT', 'TEXTAREA'].indexOf(document.activeElement.nodeName) != -1)) {
        ...
        /**
         * commented this code and added in below if as it was creating issue with ctrl keydown in hotkeys
         */
...
//      }, 0);
    ...
    if (isCtrlDown && (event.keyCode === 67 || event.keyCode === 86 || event.keyCode === 88)) {
        /**
         * added selecting textarea code here which is used for handson copy paste
         */
      that.selectNodeText(that.elTextarea);
...
WalkontableHorizontalScrollbar.prototype.getHandleSizeRatio = function (viewportCount, totalCount) {
  ...
  return (viewportCount+1) / totalCount;
};
  ...
  Typeahead.prototype = {
    ...
    , next: function (event) {
      ...
      if (!next.length) {
        ...
        //this.$menu[0].scrollTop = 0
      }
      else if(active.position().top > (this.$menu.height()-25) && this.$menu[0].scrollHeight > this.$menu[0].scrollTop) {
          this.$menu[0].scrollTop += 25;
      }
    ...
    , prev: function (event) {
      ...
      if (!prev.length) {
        ...
        //this.$menu[0].scrollTop = this.$menu[0].scrollHeight;
      }
      else if(active.position().top < 25 && this.$menu[0].scrollTop > 0) {
          this.$menu[0].scrollTop -= 25;
      }
